...
import { rollDouble, DOUBLE_PAYOUTS } from '../_shared/game-data.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!['red', 'black', 'green'].includes(choice)) return errorResponse('Escolha inválida', 400)
    ...
    const payout = DOUBLE_PAYOUTS[result.color] ?? 0
    const won = result.color === choice

    let newBalance = parseFloat(balanceAfterBet)
    let profit = 0

    if (won) {
      profit = parseFloat((betAmt * payout - betAmt).toFixed(2))
    ...
    return okResponse({ result: result.color, slot: result.slot, won, profit, newBalance })
  } catch (err) {
    console.error('casino-double error:', err)
